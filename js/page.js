(() => {
 ...
 if((catalogSlugs.includes(slug)&&!['lstk','sea-containers','ventilation'].includes(slug))||slug==='projects'){
  ...
  const fitScale=()=>{if(!previewImage.offsetHeight||!previewStage.clientHeight)return 1;return Math.max(1,Math.min(1.35,(previewStage.clientHeight-8)/previewImage.offsetHeight));};
  const paintPreview=()=>{const maxScale=fitScale();if(scale>maxScale)scale=maxScale;const maxX=Math.max(0,(previewImage.offsetWidth*scale-previewStage.clientWidth)/2),maxY=Math.max(0,(previewImage.offsetHeight*scale-previewStage.clientHeight)/2);panX=Math.max(-maxX,Math.min(maxX,panX));panY=Math.max(-maxY,Math.min(maxY,panY));previewImage.style.transform='translate('+panX+'px,'+panY+'px) scale('+scale+')';zoomReset.textContent=Math.round(scale*100)+'%';zoomOut.disabled=scale<=1;zoomIn.disabled=scale>=maxScale-.001;};
