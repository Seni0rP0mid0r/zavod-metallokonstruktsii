(() => {
 ...
 if((catalogSlugs.includes(slug)&&!['lstk','sea-containers','ventilation'].includes(slug))||slug==='projects'){
  ...
  const maxScale=6;
  const fitImage=()=>{if(!preview.open||!previewImage.naturalWidth)return;const ratio=Math.min(previewStage.clientWidth/previewImage.naturalWidth,previewStage.clientHeight/previewImage.naturalHeight);previewImage.style.width=(previewImage.naturalWidth*ratio)+'px';previewImage.style.height=(previewImage.naturalHeight*ratio)+'px';paintPreview();};
  ...
  previewStage.addEventListener('touchstart',event=>{if(event.touches.length===2){endDrag();initialPinch=Math.hypot(event.touches[0].clientX-event.touches[1].clientX,event.touches[0].clientY-event.touches[1].clientY);}}, {passive:true});previewStage.addEventListener('touchmove',event=>{if(event.touches.length===2&&initialPinch){const distance=Math.hypot(event.touches[0].clientX-event.touches[1].clientX,event.touches[0].clientY-event.touches[1].clientY);setScale(scale*(distance/initialPinch));initialPinch=distance;}},{passive:true});previewStage.addEventListener('touchend',()=>{initialPinch=null;},{passive:true});
  previewImage.addEventListener('load',fitImage);new ResizeObserver(fitImage).observe(previewStage);
  previewStage.addEventListener('dblclick',()=>setScale(scale===1?2:1));
  const previewPhotos=groups.flatMap(group=>group.items);let previewIndex=0;
  const previewNav=el('div','photo-preview-nav'),previewPrevious=el('button','','←'),previewNext=el('button','','→'),previewCount=el('span');previewPrevious.type=previewNext.type='button';previewPrevious.setAttribute('aria-label','Предыдущее фото в просмотрщике');previewNext.setAttribute('aria-label','Следующее фото в просмотрщике');previewCount.setAttribute('aria-live','polite');previewNav.append(previewPrevious,previewCount,previewNext);hint.before(previewNav);
  const browsePreview=delta=>openPreview(previewPhotos[(previewIndex+delta+previewPhotos.length)%previewPhotos.length]);previewPrevious.addEventListener('click',()=>browsePreview(-1));previewNext.addEventListener('click',()=>browsePreview(1));preview.addEventListener('keydown',event=>{if(event.key==='ArrowLeft'||event.key==='ArrowRight'){event.preventDefault();browsePreview(event.key==='ArrowRight'?1:-1);}});
  const openPreview=slide=>{endDrag();previewIndex=previewPhotos.indexOf(slide);previewCount.textContent=(previewIndex+1)+' / '+previewPhotos.length;previewImage.src=slide.src;previewImage.alt=slide.title;previewLabel.textContent=slide.title;resetPreview();if(!preview.open)preview.showModal();document.documentElement.classList.add('photo-open');fitImage();};
 ...
 if(slug==='contacts'){
  const section=el('section','contact-map-section');const contacts=el('div','contact-direct');contacts.innerHTML='<a href="tel:+79161219988">+7 (916) 121-99-88</a><a href="mailto:zhursa03@mail.ru">zhursa03@mail.ru</a>';body.prepend(contacts);section.append(el('h2','','Как нас найти'));const loc=window.EtalonLocation;
